...
{    
    ...
    // Quick error check
    // -----------------
    if (meqn<1 || maux < 0 || mpoints<1 || mpoints>3 ) 
    {
    ...
    dTensor2 phi(mpoints,MAX_PTS), phi_x(mpoints,MAX_PTS), phi_xx(mpoints,MAX_PTS);
...
#pragma omp parallel for
        for (int i=istart; i<=iend; i++)
        {
            ...
            double xc = node.get(1,1) + (double(i)-0.5)*dx;
            ...
            if( mpoints > 2) // third order terms
            {
                ...
                // compute f_x(x_k) by differentiating the basis functions:

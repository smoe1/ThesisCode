...
/* eaj note:
 * To avoid memory leaks when exceptions are thrown, one should
 * use delete only in destructors. Templated pointer classes such
 * as auto_ptr have destructors which automatically delete the
 * reference. I use this linked_ptr template class rather than
 * auto_ptr to avoid #include <memory> (8000 lines, expensive
 * to compile unless precompiled header files are used) */
template <class X> class linked_ptr
{
public:

#define NO_MEMBER_TEMPLATES /* eaj */

...
// -------------------------------------------------------------
// For the Poisson routines: (and backwards compatability?)
// -------------------------------------------------------------
void L2Project_Unst(
    const int istart, 
    const int iend, 
    ...
    {      
        ...
        for (int m=1; m<=(mpoints); m++)
        {
            ...
            for (int ma=1; ma<=maux; ma++)
            {
                ...
                for (int k=1; k<=kmax_auxin; k++)
                {
                    auxvals.set(m,ma, auxvals.get(m,ma) + phi.get(m,k) * auxin->get(i,ma,k) );
                }
            } 
        }
...
// This is identical to the above routine, with the exception that time gets
// passed into *Func, and hence needs to be passed in here as well.
void L2Project_Unst(
    const double time,
    const dTensor2* vel_vec,
    const int istart, 
        ...
        const dTensor2&,dTensor2&))
{
    ...
    {      
        ...
        // Call user-supplied function to set fvals
        Func(time, vel_vec,xpts,qvals,auxvals,fvals);
        ...
        for (int m2=1; m2<=kmax; m2++)
        {
            ...
            for (int k=1; k<=mpoints; k++)
            {
                tmp = tmp + wgts.get(k)*fvals.get(k,m1)
                        *phi.get(k,m2);
            }
            fout->set(i,m1,m2, 2.0*tmp );
        }

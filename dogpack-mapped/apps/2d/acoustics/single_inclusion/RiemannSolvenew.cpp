...
//    dTensor1& ffrl       = ffrr;
  ...
  //  cout<<"Ql_tmp= "<<Ql_tmp.get(1,1)<<endl;
  ...
  //  FluxFunc(xedge_tmp,Ql_tmp,Auxl_tmp,ftmp);
    ...
    ftmp.set(1,1,1,Auxl_tmp.get(1,1)*Ql_tmp.get(1,1));
    ftmp.set(1,1,2,Auxl_tmp.get(1,2)*Ql_tmp.get(1,1));    
  
    for (int m=1; m<=meqn; m++)
    {  ffll=ftmp.get(1,m,1)*nvec.get(1) + ftmp.get(1,m,2)*nvec.get(2);}//ffll.set(m, ftmp.get(1,m,1)*nvec.get(1) + ftmp.get(1,m,2)*nvec.get(2) );  }

  //  FluxFunc(xedge_tmp,Ql_tmp,Auxr_tmp,ftmp);

    ftmp.set(1,1,1,Auxr_tmp.get(1,1)*Ql_tmp.get(1,1));
    ftmp.set(1,1,2,Auxr_tmp.get(1,2)*Ql_tmp.get(1,1)); 

    for (int m=1; m<=meqn; m++)
    {  fflr=ftmp.get(1,m,1)*nvec.get(1) + ftmp.get(1,m,2)*nvec.get(2);}//.set(m, ftmp.get(1,m,1)*nvec.get(1) + ftmp.get(1,m,2)*nvec.get(2) );  }


    ftmp.set(1,1,1,Auxr_tmp.get(1,1)*Qr_tmp.get(1,1));
    ftmp.set(1,1,2,Auxr_tmp.get(1,2)*Qr_tmp.get(1,1));    
  
    for (int m=1; m<=meqn; m++)
    {  ffrr=ftmp.get(1,m,1)*nvec.get(1) + ftmp.get(1,m,2)*nvec.get(2);}//ffll.set(m, ftmp.get(1,m,1)*nvec.get(1) + ftmp.get(1,m,2)*nvec.get(2) );  }

  //  FluxFunc(xedge_tmp,Ql_tmp,Auxr_tmp,ftmp);

    ftmp.set(1,1,1,Auxl_tmp.get(1,1)*Qr_tmp.get(1,1));
    ftmp.set(1,1,2,Auxl_tmp.get(1,2)*Qr_tmp.get(1,1)); 

    for (int m=1; m<=meqn; m++)
    {  ffrl=ftmp.get(1,m,1)*nvec.get(1) + ftmp.get(1,m,2)*nvec.get(2);}
 ...
 //   {  ffrl.set(m, ftmp.get(1,m,1)*nvec.get(1) + ftmp.get(1,m,2)*nvec.get(2) );  }
        ...
        /*if(abs(ffrr.get(1)-ffrl.get(1))>1.0e-15 || abs(Auxr_tmp.get(1,1)-Auxl_tmp.get(1,1))>1.0e-15)
    {cout<<" ftmp= "<<ffrr.get(1)<<" "<<ffrl.get(1)<<endl;}*/
    //cout<<"ftmp= "<<ftmp.get(1,1,1)<<endl;
    ...
    switch ( mcase )
    {
        case 1:  // both s1 and s2 are positive (or both zero)
            ...
            { 
                Fl.set(m, ffll );
                Fr.set(m, fflr );
   //             cout<<"eqn= "<<m<<" "<<ffl.get(m)<<" "<<Fl.get(m)<<endl;
            }	
            break;
        ...
        case 3:  // s1 is negative and s2 is positive
            ...
            {   
                ...
                Fl.set(m, 0.5e0*(ffll + ffrr
                            + (s1+s2)*Qstar - s1*Ql.get(m) - s2*Qr.get(m)) );
                Fr.set(m, Fl.get(m) );
            }	
            break;
    }
 //   cout<<" ftmp2= "<<ffl.get(2)<<" "<<Fl.get(1)<<endl;

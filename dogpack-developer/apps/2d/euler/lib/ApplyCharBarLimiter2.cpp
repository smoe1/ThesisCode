        ...
        dTensor2&))
{
    ...
    //neix[0]=0;neix[1]=0;neix[2]=1;neix[3]=0;neix[4]=-1;
    //neiy[0]=0;neiy[1]=-1;neiy[2]=0;neiy[3]=1;neiy[4]=0;

    neix[0]=0;neix[1]=0;neix[2]=1;neix[3]=1;neix[4]=1;neix[5]=0;neix[6]=-1;neix[7]=-1;neix[8]=-1;
    neiy[0]=0;neiy[1]=-1;neiy[2]=-1;neiy[3]=0;neiy[4]=1;neiy[5]=1;neiy[6]=1;neiy[7]=0;neiy[8]=-1;


    dTensorBC4 maxW1 (mx,my,numnei, meqn,mbc);
    dTensorBC4 minW1 (mx,my,numnei, meqn,mbc);
    dTensorBC4 maxW2(mx,my,numnei, meqn,mbc);
    dTensorBC4 minW2 (mx,my,numnei, meqn,mbc);
...
#pragma omp parallel for
    ...
    for(int j=1; j <= my; j++)
    {
     ...
     for(int me=1; me <= meqn; me++)
     {
            ...
            {
                qsingle.set(me,k,q.get(i+neix[l],j+neiy[l],me,k));
            }
     ...
     {
        if(l==0)
        {aveW1.set(i,j,me,Wsingle.get(me,1));}
            for( int k=1; k <= kmax; k++ )
            { 
     ...
     {
        if(l==0)
        {aveW2.set(i,j,me,Wsingle.get(me,1));}
            for( int k=1; k <= kmax; k++ )
            { 
     ...
     for(int me=1; me <= meqn; me++)
     {
        ...
        maxW1.set(i,j,1+l,me,max1);
        minW1.set(i,j,1+l,me,min1);
        maxW2.set(i,j,1+l,me,max2);
        minW2.set(i,j,1+l,me,min2);
    ...
    //Add a tolerance.
    double dx=1.0/mx;
    double alpha=10000.0*pow(dx,1.5);
#pragma omp parallel for
    for(int i=1; i <= mx; i++)
    for(int j=1; j <= my; j++)
    { double thetae=1.0;
    for(int me=1; me <= meqn; me++)
    {
       ...
       //on our current cell.
        ...
        /*
        double difflM1=maxW1.get(i,j,1+4,me)-Q1;
        double difflm1=minW1.get(i,j,1+4,me)-Q1;

        double diffrM1=maxW1.get(i,j,1+2,me)-Q1;
        double diffrm1=minW1.get(i,j,1+2,me)-Q1;

        double diffuM1=maxW1.get(i,j,1+3,me)-Q1;
        double diffum1=minW1.get(i,j,1+3,me)-Q1;

        double diffdM1=maxW1.get(i,j,1+1,me)-Q1;
        double diffdm1=minW1.get(i,j,1+1,me)-Q1; 
        */
        ...
        for(int l=0;l<numnei;l++)
        {
            diffM1=Max(diffM1,maxW1.get(i,j,1+l,me)-Q1);
            diffm1=Min(diffm1,minW1.get(i,j,1+l,me)-Q1);
        }
        ...
        double diffcM1=maxW1.get(i,j,1+0,me)-Q1;
        double diffcm1=minW1.get(i,j,1+0,me)-Q1;
        ...
        /*
        double difflM2=maxW2.get(i,j,1+4,me)-Q2;
        double difflm2=minW2.get(i,j,1+4,me)-Q2;

        double diffrM2=maxW2.get(i,j,1+2,me)-Q2;
        double diffrm2=minW2.get(i,j,1+2,me)-Q2;

        double diffuM2=maxW2.get(i,j,1+3,me)-Q2;
        double diffum2=minW2.get(i,j,1+3,me)-Q2;

        double diffdM2=maxW2.get(i,j,1+1,me)-Q2;
        double diffdm2=minW2.get(i,j,1+1,me)-Q2;
        */
        //Compute with a tolerance
        ...
        for(int l=0;l<numnei;l++)
        {
            diffM2=Max(diffM2,maxW2.get(i,j,1+l,me)-Q2);
            diffm2=Min(diffm2,minW2.get(i,j,1+l,me)-Q2);
        }
        ...
        double diffcM2=maxW2.get(i,j,1+0,me)-Q2;
        double diffcm2=minW2.get(i,j,1+0,me)-Q2;
 ...
 //////////////////////////////////////////////////////////////////////////////////
        ...
        //double theta=Min(theta11,theta21);

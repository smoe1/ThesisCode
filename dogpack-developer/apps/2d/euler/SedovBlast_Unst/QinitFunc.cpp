...
void QinitFunc(const dTensor2& xpts, dTensor2& qvals)
{
    ...
    {
        double x = xpts.get(i,1);//2.2*(xpts.get(i,1))-1.1;
        double y = xpts.get(i,2);//2.2*(xpts.get(i,2))-1.1;

        double rad = sqrt(pow(x,2) + pow(y,2));

        double rho,u1,u2,u3,press,energy;
        //hardcode characteristic length scale here as refine grid
        double dx=0.05;double dy=dx;

...
*/
    ...
    dTensor2 avecell(NumElems,meqn);
    dTensor2 MinVal(NumElems,meqn);
    dTensor2 MaxValC(NumElems,meqn);
    dTensor2 MinValC(NumElems,meqn);
...
#pragma omp parallel for
    ...
    for(int me=1; me <= meqn; me++)
    {
        ...
        avecell.set(i,2,q.get(i,2,1));
        avecell.set(i,3,q.get(i,3,1));
        avecell.set(i,4,q.get(i,4,1));
        avecell.set(i,5,q.get(i,5,1));
...
#pragma omp parallel for
    for(int  i=1;  i <= NumPhysElems; i++)
    {
       ...
       double alpha=2000.0*pow(dx,1.5);
       double alphat=500.0*pow(dx,1.5);
       double alphac=50000.0*pow(dx,1.5);
       ...
       double yave=(y1+y2+y3)/3.0;
       double xave=(x1+x2+x3)/3.0;
    bool nstep=(yave>0.25 );
    bool nstepleft=(yave<=0.25 && xave<=0.15*3.0);
    bool corner=(xave>0.15*3.0 && xave<=0.25*3.0 && yave<=0.25);
    bool step=1-(nstep || nstepleft || corner);//~(nstep || corner);
    double alpha1=nstep*alpha+nstepleft*alphat+corner*alphac+0.0;
    nstep=1-step;
...

    {
       for(int me=1; me <= meqn; me++)
       {
           //double Q1=avecell.get(i,me);
           double Q1=nstep*avecell.get(i,me)+step*q.get(i,me,1);
           double diffM=0.0;
           double diffm=0.0;
           ...
           for(int e=1;e<=3;e++)
           {
              ...
              index=(ileft==i)*(iright-ileft)+ileft;
              /*if(ileft==i){index=iright;}
              else{index=ileft;}*/
              //diffM=Max(diffM,MaxVal.get(index,me)-Q1);
              //diffm=Min(diffm,MinVal.get(index,me)-Q1);
              diffM=Max(diffM,nstep*MaxVal.get(index,me)+step*q.get(index,me,1)-Q1);
              diffm=Min(diffm,nstep*MinVal.get(index,me)+step*q.get(index,me,1)-Q1);
           }
           /*
           for(int e=1;e<=3;e++)
           {
           ...
           double diffcM=nstep*MaxVal.get(i,me)+step*MaxValC.get(i,me)-Q1;
           double diffcm=nstep*MinVal.get(i,me)+step*MaxValC.get(i,me)-Q1;

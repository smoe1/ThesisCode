...
void SetBndFlux(const dTensor1& nvec, const dTensor1& xedge,
    ...
    for (int i=1; i<=NumEdges; i++)
    {
      ...
      if(i1<=NumPhysElems && i2<=NumPhysElems)  
      {
        ...
        for (int ell=1; ell<=dogParams.get_space_order(); ell++)
        {
            ...
            if(Max(fabs(s1),fabs(s2))>smax_globeV[thread])
            {smax_globeV[thread] = Max(fabs(s1),fabs(s2));}
    ...
    for (int i=0; i<nthreads; i++)
    {
     smax_globe=Max(smax_globe,smax_globeV[i]);
    }
    ...
    for (int i=1; i<=NumEdges; i++)
    {
      ...
      if(i1<=NumPhysElems && i2<=NumPhysElems)  
      {
        ...
        for (int ell=1; ell<=dogParams.get_space_order(); ell++)
        {
            ...
            if (i1>NumPhysElems && i2<=NumPhysElems )
            {iside=1;}
            else if (i2>NumPhysElems && i1<=NumPhysElems)
            {iside=2;}

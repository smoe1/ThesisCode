...
#pragma omp parallel for
    ...
    //--------------------------------------------------------------//
    dTensor2 MaxVal(NumElems,meqn);MaxVal.setall(0.0);
    dTensor2 avecell(NumElems,meqn);avecell.setall(0.0);
    dTensor2 MinVal(NumElems,meqn);MinVal.setall(0.0);
    dTensor2 MaxValC(NumElems,meqn);MaxValC.setall(0.0);
    dTensor2 MinValC(NumElems,meqn);MinValC.setall(0.0);
...
#pragma omp parallel for
    for(int  i=1;  i <= NumPhysElems; i++)
    {
       ...
       bool nstep=((y1+y2+y3)/3.0>0.3 || (x1+x2+x3)/3.0<0.75);
       bool step=1-nstep;

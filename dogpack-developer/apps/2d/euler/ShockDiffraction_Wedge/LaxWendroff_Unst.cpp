...
void SetBndFlux(const dTensor1& nvec, const dTensor1& xedge,
    ...
    ofstream outFile; 
    outFile.open("residual.txt", ios::out | ios::app);
    ...
    for (int i=1; i<=NumEdges; i++)
    {
     ...
     for(int e=1;e<=3;e++)
       ...
       {cout<<" problem3 "<<i<<" "<<ileft<<" "<<iright<<endl;exit(1);}
        ...
        int iside  = -1;
        if (ileft>NumPhysElems)
        {iside=1;}
        else if (iright>NumPhysElems)
        {iside=2;}
...
#pragma omp parallel for
        ...
        for (int k=1; k<=kmax; k++)
        {
            ...
            Lstar.set(i,m,k, tmp );	    
            max1=Max(max1,fabs(ElemFluxIntegral.get(i,m,1) - EdgeFluxIntegral.get(i,m,1)));  
        }
    }

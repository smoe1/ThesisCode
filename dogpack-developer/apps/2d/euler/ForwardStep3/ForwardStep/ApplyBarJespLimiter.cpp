        ...
        dTensor2&))
{
    ...
    dTensorBC3 MaxVal(mx,my,meqn,mbc); MaxVal.setall(-1.0e10);
    dTensorBC3 MinVal(mx,my,meqn,mbc); MinVal.setall(1.0e10);
    dTensorBC3 avecell (mx,my, meqn,mbc);
    dTensorBC2 MTheta(mx,my,mbc); MTheta.setall(1.0);

    double dx1=1.0/mx;
    double dy1=1.0/my;
    double dx=sqrt(0.5*(dx1*dx1+dy1*dy1));
...
#pragma omp parallel for
    ...
    for(int j=1-mbc; j <= my+mbc; j++)
    {
         ...
         /*double mine     = MinVal.get(i,j,5);
         double maxe     = MaxVal.get(i,j,5);
         double minrho   = MinVal.get(i,j,1);
         double maxrho   = MaxVal.get(i,j,1);
         double minurho   = MinVal.get(i,j,2);
         double minvrho   = MinVal.get(i,j,3);
         double minwrho   = MinVal.get(i,j,4);
         double maxurho   = MaxVal.get(i,j,2);
         double maxvrho   = MaxVal.get(i,j,3);
         double maxwrho   = MaxVal.get(i,j,4);*/
         double thetae   = 1.0;
         double thetarho = 1.0;
...
#pragma omp parallel for
    for(int i=1; i <= mx; i++)
    for(int j=1; j <= my; j++)
    { double thetae=MTheta.get(i,j);int detector=1;
    {
    for(int me=1; me <= meqn; me++)
    {
        double Q1=avecell.get(i,j,me);//q.get(i,j,me,1);

...
#pragma omp parallel for
        ...
        for (int j=1; j<=my+1; j++)
        {
           if(i<=istep || j>=jstep)
           {
            ...
            for (int ell=1; ell<=space_order; ell++)
            {
                ...
                int iside=0;
                //if(i==1){iside=1;SetBndFluxLxW(nvec,xedge,Ql,Qr,ffl,ffr,Auxl,Auxr,iside);}
                if (i==istep && j < jstep){
                iside=2;SetBndFluxLxW(nvec,xedge,Ql,Qr,ffl,ffr,Auxl,Auxr,iside);
                }
    ...
    nvec.set(1, 0.0e0 );
    nvec.set(2, 1.0e0 );
/*            cout<<" Vo1 coefficients k= ";
            for( int k=1; k <= kmax; k++ )
            {cout<<q.get(11,3,3,k)<<" k= ";}
            cout<<endl;
*/
//SetBndValuesY(q, aux);
/*            cout<<" Vo2 coefficients k= ";
            for( int k=1; k <= kmax; k++ )
            {cout<<q.get(11,3,3,k)<<" k= ";}
            cout<<endl;

            cout<<" Vo4 coefficients k= ";
            for( int k=1; k <= kmax; k++ )
            {cout<<q.get(11,4,3,k)<<" k= ";}
            cout<<endl;
*/
        ...
        for (int j=1; j<=my+1; j++)
        {
           if(i<=istep || j>=jstep)
           {
            ...
            xedge.set(2, ylower + (double(j)-1.0)*dy );
                ...
                if(j==1){
                iside=1;SetBndFluxLxW(nvec,xedge,Ql,Qr,ffl,ffr,Auxl,Auxr,iside);
                }
            ...
            if(Ql.get(1)<1.0e-14 || Qr.get(1)<1.0e-14){
            cout<<"qval= "<<q.get(i,j-1,1,1)<<" "<<q.get(i,j-1,1,2)<<endl;
            cout<<"i= "<<i<<" j= "<<j<<" density here "<<Ql.get(1)<<endl;exit(1);}
            const double smax_edge = RiemannSolveLxW(
                nvec, xedge, Ql, Qr, Auxl, Auxr, ffl, ffr, Fl, Fr);
            smax.set(i, j-1, 2,  Max(dx*smax_edge, smax.get(i, j-1, 2)) );
            smax.set(i, j,   2,  Max(dx*smax_edge, smax.get(i, j,   2)) );
            /*if(i==11 && j==4){
            cout<<"Ql= "<<Ql.get(1)<<" "<<Ql.get(2)<<" "<<Ql.get(3)<<" "<<Ql.get(4)<<" "<<Ql.get(5)<<endl;
            cout<<"Qr= "<<Qr.get(1)<<" "<<Qr.get(2)<<" "<<Qr.get(3)<<" "<<Qr.get(4)<<" "<<Qr.get(5)<<endl;
            cout<<"Fluxes "<<Fr.get(1)<<" "<<Fr.get(2)<<" "<<Fr.get(3)<<" "<<Fr.get(4)<<" "<<Fr.get(5)<<endl;}
            */
...
#pragma omp parallel for
      ...
      for (int m=1; m<=meqn; m++)
    {
        ...
        //if(i==11 && j==4){cout<<"hereP "<<F2<<" "<<F1<<" "<<G2<<" "<<G1<<" "<<(half_dx_inv*(F2-F1) + half_dy_inv*(G2-G1))<<endl;} 
...
#pragma omp parallel for
    ...
    for (int m=1; m<=mlength; m++)
    {
        ...
        //if(i==11 && j==4){cout<<"here "<<F2<<" "<<F1<<" "<<G2<<" "<<G1<<" "<<(half_dx_inv*(F2-F1) + half_dy_inv*(G2-G1))<<endl;} 

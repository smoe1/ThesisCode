...
#pragma omp parallel for
    ...
    double alphat=1000.0*pow(dx,1.5);
    double alphac=1000.0*pow(dx,1.5);
...
#pragma omp parallel for
    ...
    //bool nstep=(i<istep+10 || j>jstep+10 );
    //bool nstep=(j>my/4 );
    //bool nstep=(j>12*my/40);
    //bool nstepleft=(j<=12*my/40 && i<=3*mx/20);
    //bool corner=(i>3*mx/20 && i<=2*mx/8 && j<=12*my/40); 
    bool nstep=(j>8*my/40+6);
    bool nstepleft=(j<=8*my/40+6 && i<=3*mx/20);
    bool corner=(i>3*mx/20 && i<=2*mx/8 && j<=8*my/40+6); 
    //bool step=0;//1-(nstep || nstepleft || corner);//~(nstep || corner);
    //bool step=1-(nstep || nstepleft || corner);//~(nstep || corner);
    bool step=1-(nstep || corner || nstepleft);
    double alpha1=nstep*alpha+nstepleft*alphat+corner*alphac+step*alphas;
    nstep=1-step;
    ...
    for(int me=1; me <= meqn; me++)
    {
        ...
        */
        ...
        {
            thetam1=step*Min(thetam1,phi_func(diffm/diffcm))+nstep*Min(1.0,diffm/diffcm);
        }

...
#pragma omp parallel for
    ...
    double alphac=5000.0*pow(dx,1.5);
    double alphas=100.0*pow(dx,1.5);
...
#pragma omp parallel for
    ...
    //bool nstep=(i<istep+10 || j>jstep+10 );
    bool nstep=(j>my/4 );
    bool nstepleft=(j<=my/4 && i<=3*mx/20);
    bool corner=(i>3*mx/20 && i<=2*mx/8 && j<=my/4); 
    bool step=1-(nstep || nstepleft || corner);//~(nstep || corner);
    nstep=1-corner;
    //double alpha1=nstep*alpha+nstepleft*alphat+corner*alphac+alphas*step;
    double alpha1=nstep*alpha+corner*alphac;
    nstep=1-step;
    ...
    for(int me=1; me <= meqn; me++)
    {
        ...
        */
        ...
        //double diffcm=nstep*MinVal.get(i,j,me)+step*MaxValC.get(i,j,me)-Q1;

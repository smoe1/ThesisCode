...
void SetBndValues(dTensorBC4& q, dTensorBC4& aux)
{
    ...
    // ********************************************************************* //
    // Reset every value inside the step to a junk value.
    ...
    for (int m=1; m<=meqn; m++)
    {
        for(int k=1;k<=1;k++)
        {
            double tmp = q.get(mx,j,m,k);//*xi_sign[k-1];
            q.set(i,j,m,k, tmp );
        }
        //for (int k=2; k<=kmax; k++)
        //{ q.set(i,j,m,k, 0.0 ); }
...
*/
    ...
    {for (int m=1; m<=meqn; m++)
    {
        for(int k=1;k<=kmax;k++)
        {
            //double tmp = eta_sign[k-1]*q.get(i,my,m,k);
            double tmp = q.get(i,my,m,k);                    
            q.set(i,j,m,k, tmp );
        }
    }
    ...
    //
    // The last two of these regions are hard-surface boundaries.  The first
    ...
    for( int j = 0; j >= 1-mbc; j-- )
    {
        for(int k=1; k<=kmax; k++)
        {
            for (int m=1; m<=meqn; m++)
            {
                //double tmp = eta_sign[k-1]*q.get(i, 1-j, m,k);                    
                double tmp = q.get(i, 1-j, m,k);                    
                q.set(i,j,m,k, tmp );
            }
    ...
    for (int j = 1; j < jstep; j++ )
    {
        for (int m=1; m<=meqn; m++)
        {
            for(int k=1;k<=kmax;k++)
            {
                ...
                double tmp = q.get(2*istep-1-i, j, m,k);                    
                q.set(i,j,m,k, tmp );
            }
        }
        /*for(int k=1;k<=kmax;k++)
    ...
    // Segment 3 (Changed on 2/23/2015 -SM)
    for (int i=istep; i<= mx+mbc; i++)
    ...
    for (int k=1; k<=kmax; k++)
    {
        for (int m=1; m<=meqn; m++)
        {
            ...
            double tmp = q.get(i, 2*jstep-1-j, m,k);                    
            q.set(i,j,m,k, tmp );
            //q.set(i,j,m,1,q.get(i,2*jstep-1-j,m,1));
        }
            // Flip the momemtum u2:
            //q.set(i,j,3,k, -q.get(i,j,3,k) );
    }
...
void SetBndValuesY(dTensorBC4& q, dTensorBC4& aux)
{
    ...
    {
      /*
        if(i==11 && j==3){cout<<" HERE2! "<<2*jstep-1-j<<endl;

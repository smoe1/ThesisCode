...
void ApplyPosLimiterSpecial(const dTensorBC4& aux, dTensorBC4& q)
{
        ...
        for(int j=1-mbc; j <= my+mbc; j++)
            {
                ...
                else if( m < 0.0){ theta = Min( thetam, fabs( (eps-Q1) / (m-Q1) ) ); }
                ...
                /*for(int mp=1; mp <= mpoints; mp++)
                {
                    ...
                    double press=rhonow*enow-0.5*(mxnow*mxnow+mynow*mynow+mznow*mznow);
                   if (press<0.0){
              cout<<"thetam "<<thetam<<" theta "<<theta<<" m= "<<m<<"  =previous, ave= "<<ave<<" problem here "<<press<<endl;
                           exit(1); }
                }*/
...
void SetPositivePointsSpecial(const int& space_order, dTensor2& spts)
{
...
void SamplePhiAtPositivePointsSpecial(const int& space_order, 

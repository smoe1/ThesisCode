...
void DogSolverCart2::DogSolveLxW(double tstart, double tend)
{
    ...
    while (t<tend)
    {
        ...
        while (m_accept==0)
        {
            ...
            dogParams.set_time(told);      // TODO - `What's this TODO
                                           // statement for? -DS (6/16/2014)
            set_dt(dt);
        ...
        /*void LaxWendroff(double dt, 
          ...
          LaxWendroff(dt, aux, qnew, L, smax);*/
            void LaxWendroff(double dt, 
            ...
            SetBndValues(qnew, aux);
            // -------------------------------------------------------------- //
            // TODO: Figure out how we want to handle this flag, just as in 
            //       the 1d case.  -DS.
            //
            if( dogParams.using_positive_limiter() )
            {   
...
// Take a single ("Euler") time step of length dt.
void StepLxW( double dt, const dTensorBC4& qold,
    const dTensorBC4& L, dTensorBC4& qnew )
{

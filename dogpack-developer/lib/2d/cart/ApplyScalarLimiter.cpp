...
// A limiter for the 2D code has been implemented for systems up to third-order
// only.  This is an implementation for the scalar case, where we can construct
// somthing that goes all the way up to fifth-order.
//
// This routine is called when limiters are turned on, the type of limiter is
// set to "moment", and you are solving a scalar equation.
//
// See also: Limiters.cpp.

    ...
    dTensorBC4& Lstar, dTensorBC3& smax)
{
    ...
    // --------------------------------------------------------------------- //
    void L2ProjectLxW( const int mterms, double alpha, double beta_dt,
        const double charlie_dt,
...
// Same version as above, but this one allows for a three stage method
void LaxWendroffTD(double dt, 
    double alpha1,  double beta1,      // parameteres used for each derivative.
    dTensorBC4& aux1, dTensorBC4& q1,    // set bndy values modifies these
    double alpha2,  double beta2,      // parameteres used for each derivative.
    dTensorBC4& aux2, dTensorBC4& q2,    // set bndy values modifies these
    double alpha3,  double beta3,      // parameteres used for each derivative.
    dTensorBC4& aux3, dTensorBC4& q3,    // set bndy values modifies these
    dTensorBC4& Lstar, dTensorBC3& smax)
{
...
#pragma omp parallel for
    ...
    // ---------------------------------------------------------------------- //
    // Project and add the contributions from the third term
    // ---------------------------------------------------------------------- //
    L2ProjectLxW( 2, alpha3, dt*beta3, 0.,
        1-mbc, mx+mbc, 1-mbc, my+mbc,
        space_order, space_order, space_order,
        &q2, &aux2, &Ftmp, &Gtmp, &FluxFunc, &DFluxFunc, D2FluxFunc );
#pragma omp parallel for

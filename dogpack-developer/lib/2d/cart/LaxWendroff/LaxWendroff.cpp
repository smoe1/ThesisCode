...
//
// This routine constructs the 'time-averaged' flux functions F and G using the
...
//    L(q) := -F_x - G_y.
//
// It is assumed that boundary conditions have already been applied before
// calling this routine.
//
// See also: ConstructL.
void LaxWendroff(double dt, const dTensorBC4& aux, const dTensorBC4& q,
    dTensorBC4& Lstar, dTensorBC3& smax)
{
    ...
    dTensorBC3 auxI(mx, my, maux, mbc);
    ...
    for (int k=2; k<=kmax; k++)
    {
            ...
            Lstar.fetch(i,j,m,k) -= 1.0*(half_dx_inv*(F2-F1) + half_dy_inv*(G2-G1));
      }
    }

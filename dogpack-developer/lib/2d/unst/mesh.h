...
class mesh
{
 public:
  ...
  const int& get_bnd_edge(int i) const;
  ...
  const double& get_LowerLeft(int index) const;
  const double& get_UpperRight(int index) const;
  ...
  const void compute_min_max_edge_length();
  ...
  // Detailed comparisons of 2 mesh objects (useful for debugging)
  bool Compare(const mesh& tmp,const bool verbose) const;

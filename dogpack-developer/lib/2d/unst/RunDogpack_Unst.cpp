...
int RunDogpack_Unst(const char* outputdir)
{
    ...
    int garbage;
    garbage = fscanf(mesh_file,"%i",&NumElems);  
    assert( fgets(buffer, sizeof buffer, mesh_file) != NULL );
    garbage = fscanf(mesh_file,"%i",&NumPhysElems);  
    assert( fgets(buffer, sizeof buffer, mesh_file) != NULL );
    garbage = fscanf(mesh_file,"%i",&NumGhostElems);  
    assert( fgets(buffer, sizeof buffer, mesh_file) != NULL );
    garbage = fscanf(mesh_file,"%i",&NumNodes);  
    assert( fgets(buffer, sizeof buffer, mesh_file) != NULL );
    garbage = fscanf(mesh_file,"%i",&NumPhysNodes);  
    assert( fgets(buffer, sizeof buffer, mesh_file) != NULL );
    garbage = fscanf(mesh_file,"%i",&NumBndNodes);  
    assert( fgets(buffer, sizeof buffer, mesh_file) != NULL );
    garbage = fscanf(mesh_file,"%i",&NumEdges);  
    assert( fgets(buffer, sizeof buffer, mesh_file) != NULL );
    garbage = fscanf(mesh_file,"%i",&NumBndEdges);  
    assert( fgets(buffer, sizeof buffer, mesh_file) != NULL );
    fclose(mesh_file);
    ...
    dTensor3 qtest(NumElems,meqn,kmax);
    ...
    for (int n=nstart+1; n<=nout; n++)
    {
        ...
        {
            DogSolveLxW_Unst(Mesh,EdgeData,aux,qold,qnew,tstart,tend,outputdir);
        }

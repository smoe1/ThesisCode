			    ...
			    dTensor1& Fr)
{
  ...
  dTensor3 ftmp(1,meqn,3);
  double s1,s2,Qstar;
  dTensor2 xedge_tmp(1,3),Ql_tmp(1,meqn),Qr_tmp(1,meqn);
  ...
  for (m=1; m<=meqn; m++)
    {  ffl.set(m, ftmp.get(1,m,1)*nvec.get(1) + ftmp.get(1,m,2)*nvec.get(2) + ftmp.get(1,m,3)*nvec.get(3) );  }
    
  FluxFunc(xedge_tmp,Qr_tmp,Auxr_tmp,ftmp);
  for (m=1; m<=meqn; m++)
    {  ffr.set(m, ftmp.get(1,m,1)*nvec.get(1) + ftmp.get(1,m,2)*nvec.get(2) + ftmp.get(1,m,3)*nvec.get(3) );  }

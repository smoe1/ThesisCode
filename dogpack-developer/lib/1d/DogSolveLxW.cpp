...
{  
    ...
    while (t<tend)
    {
        ...
        while (m_accept==0)
        {
            ...
            if( dogParams.using_positive_limiter() )
            { 
                // TODO - decide on how we want to pass this flag in.
                // this is slightly buggy, because the moment_limiter flag
                // is used to describe two things: positivity (name says it all), 
                // and moment (shock-capturing) limiting.  The latter is necessary
                // for shock capturing.  12/15/2014 -DS

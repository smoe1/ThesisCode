...
function val = sample_QNi111(c, state_coef); val = get_QNsij(c, state_coef, 'i',1,1,1); end
function val = sample_QNi112(c, state_coef); val = get_QNsij(c, state_coef, 'i',1,1,2); end
function val = sample_QNi113(c, state_coef); val = get_QNsij(c, state_coef, 'i',1,1,3); end
function val = sample_QNi122(c, state_coef); val = get_QNsij(c, state_coef, 'i',1,2,2); end
function val = sample_QNi123(c, state_coef); val = get_QNsij(c, state_coef, 'i',1,2,3); end
function val = sample_QNi133(c, state_coef); val = get_QNsij(c, state_coef, 'i',1,3,3); end
function val = sample_QNi222(c, state_coef); val = get_QNsij(c, state_coef, 'i',2,2,2); end
function val = sample_QNi223(c, state_coef); val = get_QNsij(c, state_coef, 'i',2,2,3); end
function val = sample_QNi233(c, state_coef); val = get_QNsij(c, state_coef, 'i',2,3,3); end
function val = sample_QNi333(c, state_coef); val = get_QNsij(c, state_coef, 'i',3,3,3); end
function QNsij = get_QNsij(c, state_coef, species,i,j,k)
  stateIndices_s = c.s.stateVarMap.stateIndices_s;
  idx = c.s.tensorIdxMap.linearIdx3(i,j,k);
  QNs_ = ['QN' species];
  stateIdx=stateIndices_s.(QNs_)(idx);
  QNsij_coef = state_coef(:,:,:,stateIdx);
  QNsij = sample_state_cart2(QNsij_coef, c.s.sample_rate);
end

% get velocity gradient
function val = get_Di_uj(c, state_coef, species,i,j, sample_rate)
  switch i
  case 1
    val = get_usjx(c, state_coef, species, j);
  case 2
    val = get_usjy(c, state_coef, species, j);
  case 3
    val = sample_zeros(c,1);
  otherwise
    error(['unsupported index: ' num2str(i)]);
  end
end

% get rate of strain
% Sym2(D u) = (D u + (D u)^T)
function val = get_sym2Du_ij(c, state_coef, species,i,j)
  % compute Di_uj
  Di_uj = get_Di_uj(c, state_coef, species,i,j);
  Dj_ui = get_Di_uj(c, state_coef, species,j,i);
  val = (Di_uj + Dj_ui);
end

function out = get_mag6(M11, M12, M13, M22, M23, M33);
  mag2 = M11.^2 + M12.^2 + M13.^2 + M22.^2 + M23.^2 + M33.^2;
  out = sqrt(mag2);
end

function out = sample_magdpi5i(c, coef); out = get_magdpi5s(c, coef, 'i'); end
function out = sample_magdpi5e(c, coef); out = get_magdpi5s(c, coef, 'e'); end
function out = get_magdpi5s(c, coef, species)
  [dpi11, dpi12, dpi13, dpi22, dpi23, dpi33] ...
    = get_dpi5s(c, coef, species);
  out = get_mag6(dpi11, dpi12, dpi13, dpi22, dpi23, dpi33);
end

function out = sample_dpi5i3(c, coef); out = get_dpi5s3(c, coef, 'i'); end
function out = sample_dpi5e3(c, coef); out = get_dpi5s3(c, coef, 'e'); end
function eig3 = get_dpi5s3(c, coef, species)

  [dpi11, dpi12, dpi13, dpi22, dpi23, dpi33] ...
    = get_dpi5s(c, coef, species);

  [eig1, eig2, eig3] = get_eigs(dpi11, dpi12, dpi13, dpi22, dpi23, dpi33);
end

function [dpi11, dpi12, dpi13, dpi22, dpi23, dpi33]  ...
  = get_dpi5s(c, coef, species)

  [dstrain11, dstrain12, dstrain13, dstrain22, dstrain23, dstrain33] ...
    = get_dstrain(c, coef, species);
  tau = get_iso_period_s(c, coef, species);
  dpi11 = tau.*dstrain11;
  dpi12 = tau.*dstrain12;
  dpi13 = tau.*dstrain13;
  dpi22 = tau.*dstrain22;
...
function [dstrain11, dstrain12, dstrain13, dstrain22, dstrain23, dstrain33] ...
  = get_dstrain(c, state_coef, species)

  [strain11, dstrain12, dstrain13, strain22, dstrain23] ...
    = get_strain(c, state_coef, species);
  divu = strain11 + strain22;
  divu_3 = (1./3.)*divu;
  dstrain11 = strain11 - divu_3;
  dstrain22 = strain22 - divu_3;
  dstrain33 = - divu_3;
end

% get rate of strain tensor
% Sym(D u) = (D u + (D u)^T)/2
function [strain11, strain12, strain13, strain22, strain23] ...
  = get_strain(c, state_coef, species)

  strain11 = get_Di_uj(c, state_coef, species,1,1);
  strain12 = 0.5*(get_Di_uj(c, state_coef, species,1,2) + ...
                  get_Di_uj(c, state_coef, species,2,1));
  strain13 = get_Di_uj(c, state_coef, species,1,3);
  strain22 = get_Di_uj(c, state_coef, species,2,2);
  strain23 = get_Di_uj(c, state_coef, species,2,3);
end

function val = sample_dpi5e11(c, coef); val = get_dpi5sMN(c, coef, 'e',1,1); end
function val = sample_dpi5e12(c, coef); val = get_dpi5sMN(c, coef, 'e',1,2); end
function val = sample_dpi5e13(c, coef); val = get_dpi5sMN(c, coef, 'e',1,3); end
function val = sample_dpi5e22(c, coef); val = get_dpi5sMN(c, coef, 'e',2,2); end
function val = sample_dpi5e23(c, coef); val = get_dpi5sMN(c, coef, 'e',2,3); end
function val = sample_dpi5e33(c, coef); val = get_dpi5sMN(c, coef, 'e',3,3); end

function val = sample_dpi5i11(c, coef); val = get_dpi5sMN(c, coef, 'i',1,1); end
function val = sample_dpi5i12(c, coef); val = get_dpi5sMN(c, coef, 'i',1,2); end
function val = sample_dpi5i13(c, coef); val = get_dpi5sMN(c, coef, 'i',1,3); end
function val = sample_dpi5i22(c, coef); val = get_dpi5sMN(c, coef, 'i',2,2); end
function val = sample_dpi5i23(c, coef); val = get_dpi5sMN(c, coef, 'i',2,3); end
function val = sample_dpi5i33(c, coef); val = get_dpi5sMN(c, coef, 'i',3,3); end

% get the MN component of the 5-moment pressure shape deviator of species s
function val = get_dpi5sMN(c, coef, species,M,N)
  straindev2 = get_straindev2sMN(c, coef, species,M,N);
  tau = get_iso_period_s(c, coef, species);
  val = tau.*straindev2;
>>>>>>> committing matlab code to display 20-moment results:matlab/plasma/@GEMplotter/sample_var_from_state.m
end

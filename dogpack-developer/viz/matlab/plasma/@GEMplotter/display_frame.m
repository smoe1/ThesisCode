function display_frame(c, time, data, var_name, varargin)
  ...
  while idx <= numel(varargin)
    switch lower(varargin{idx})
      case 'fig'; idx=idx+1;
        fig=varargin{idx};
        raise_fig=1;
      case 'position'; idx=idx+1;
        Position=varargin{idx}
  ...
  if(~exist('xlabel_str','var'))
    xlabel_str = get_xlabel(c,use_long_labels);
  end
  set_xlabel(c,xlabel_str);
  set_title(c,display_string,time,use_long_labels);
  hold off;
  if(numel(printarg)>0)
    frame_number = c.s.stateInfo.frame_number
    filename = ['/Users/evanjohnson/figures/' ...
                printarg num2str(frame_number)];
...
function xlabel_str = get_xlabel(c,use_long_labels)
    params = c.s.params;
    %if(c.s.do_rescale)
    %  if(use_long_labels)
...
function set_xlabel(c,xlabel_str)
    %params = c.s.params;
    h = xlabel(xlabel_str);

function h=pdeplot_tmp(p,e,t,p1,v1,p2,v2,p3,v3,p4,v4,p5,v5,p6,v6,p7,v7,p8,v8,p9,v9,p10,v10,p11,v11,p12,v12,p13,v13,p14,v14)
...
for i=1:2:nargs-4,
  ...
  case 'xydata'
    xydata = Value;
    if isstr(xydata),
      error('xydata must be a vector.')
    end
  case 'xystyle'
    xystyle=lower(Value);
    if ~isstr(xystyle),
      error('xystyle must be a string.')
    elseif ~(strcmp(xystyle,'off') | strcmp(xystyle,'flat') | ...
          strcmp(xystyle,'interp')),
      error('xystyle must be off | flat | interp')
    end
  case 'zdata'
    zdata = Value;
    if isstr(zdata),
      error('zdata must be a vector.')
    end
  case 'zstyle'
    zstyle=lower(Value);
    if ~isstr(zstyle),
      error('zstyle must be a string.')
    elseif ~(strcmp(zstyle,'off') | strcmp(zstyle,'continuous') | ...
          strcmp(zstyle,'discontinuous')),
      error('zstyle must be off | continuous | discontinuous')
    end
  case 'flowdata'
    flowdata = Value;
    if isstr(flowdata),
      error('flowdata must be a matrix.')
    end
  case 'flowstyle'
    flowstyle=lower(Value);
    if ~isstr(flowstyle),
      error('flowstyle must be a string.')
    elseif ~(strcmp(flowstyle,'off') | strcmp(flowstyle,'arrow'))
      error('flowstyle must be off | arrow')
    end
  case 'colormap'
    if isstr(Value),
      cmap = lower(Value);
  ...
  case 'intool'
    intool = lower(Value);
    if ~isstr(intool),
      error('intool must be a string.')
    elseif ~(strcmp(intool,'off') | strcmp(intool,'on'))
      error('intool must be off | on')
    end
  case 'xygrid'
    xygrid = lower(Value);
    if ~isstr(xygrid),
      error('xygrid must be a string.')
    elseif ~(strcmp(xygrid,'off') | strcmp(xygrid,'on'))
      error('xygrid must be off | on')
    end
  case 'mesh'
    mh = lower(Value);
    if ~isstr(mh),
      error('mesh must be a string.')
    elseif ~(strcmp(mh,'off') | strcmp(mh,'on'))
      error('mesh must be off | on')
    end
  case 'colorbar'
    cbar = lower(Value);
    if ~isstr(cbar),
      error('colorbar must be a string.')
    elseif ~(strcmp(cbar,'off') | strcmp(cbar,'on'))
      error('colorbar must be off | on')
    end
  case 'title'
    title = Value;
    if ~isstr(title),
  ...
  case 'gridparam'
    gridparam = Value;
    if isstr(gridparam),
      error('gridparam must be a matrix [tn; a2; a3]')
  ...
  case 'contour'
    cont = lower(Value);
    if ~isstr(cont),
      error('contour must be a string.')
    elseif ~(strcmp(cont,'off') | strcmp(cont,'on'))
      error('contour must be off | on')
    end
  otherwise
...
if intool,
  ...
  if isempty(ax),
    error('Intool plot option on. Couldn''t find PDETool axes.')
  end
...
if plotxy,
  ...
  elseif xys(1)==ntri
    ...
    error('length of xydata must equal number of nodes or number of triangles')
  end
end
if plotz,
  zs=size(zdata);
  ...
  elseif zs(1)==ntri
    ...
    error('length of zdata must equal number of nodes or number of triangles')
  end
end
if plotflow,
  flows=size(flowdata);
  if flows(2)==ntri,
    if flows(1)<2,
      error('flowdata must be a matrix with two rows (columns).')
    else
  ...
  elseif flows(1)==nnode,
    if flows(2)<2,
      error('flowdata must be a matrix with two columns.')
    else
      flowdata=flowdata(:,1:2);
    end
  else
    error('flowdata must be of size 2-by-no of triangles or no of nodes-by-2.')
  end
end
...
if strcmp(cont,'on') & plotxy,
  ...
  if strcmp(xygrid,'on'),
    ...
    if ~intool | plotz,
      ...
      set(pde_fig,'CurrentAxes',ax)
      hold on
      [unused,hhc]=contour(x,y,zdata,levels);
      % plot geometry boundaries:

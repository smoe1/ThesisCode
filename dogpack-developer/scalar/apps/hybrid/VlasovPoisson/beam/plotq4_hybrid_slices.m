...
%     qtmp(1, :, : ) = q_data( TriNum, : , : );
      ...
      % Cartesian points (uniform to every element!)
      s2dcart_tmp = zeros( local_pts_per_dir, 2 );
      s2dcart_tmp(:,1) = -1+(2*(1:points_per_dir)-1)/points_per_dir;

      % Now, paste these elements together:
      s4d_tmp = zeros( local_pts_per_dir^2, 4 );

% This works when num_pts == 1 
      %s2d_tmp = zeros( local_pts_per_dir, 2 );
...
%     s4d_tmp(:,  3) = -1+(2*(1:points_per_dir)-1)/points_per_dir;
      ...
      k_tmp = 0;
      for n1=1:local_pts_per_dir
      for n2=1:local_pts_per_dir
          k_tmp = k_tmp+1;
          s4d_tmp( k_tmp, 1 ) = s2d_tmp( n1, 1 );
          s4d_tmp( k_tmp, 2 ) = s2d_tmp( n1, 2 );
          s4d_tmp( k_tmp, 3 ) = -1+(2*(n2)-1)/points_per_dir;
      end
      end
...
% TODO - this area is not correct %
      ...
      for vx=1:mx
        ...
        vxc = v_xlow + (vx-0.5)*dvx;

        k_tmp = 0;
        for mp1=1:(local_pts_per_dir)
        for mp2=1:(local_pts_per_dir)
          k_tmp = k_tmp + 1;
          xx   (index+mp1, index_vx+mp2) = x(mp1);
          yy   (index+mp1, index_vx+mp2) = vxc + 0.5*dvx*s4d_tmp( k_tmp, 3 );
          qpts (index+mp1, index_vx+mp2) = qvals(k_tmp);
        end
        end
        index_vx = index_vx + local_pts_per_dir;
      end

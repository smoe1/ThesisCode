...
%% ------------------------------------------------------------------------- %%
  ...
  tnode_old = tnode;
  if (points_per_dir>1)
    % TODO - THIS CALL IS BROKEN.  What's the correct way to call this?
...
% LegVals = GetHybr4Legendre(kmax4d, z4d_unst);
  ...
  while(nf~=-1)
    ...
    if (nf~=-1)
      ...
      qsoln = zeros( grid_struct.NumElems, kmax4d );
      for i=1:grid_struct.NumElems
      for k=1:kmax4d
        qsoln(i,k) = q_data( ceil(grid_struct.mx/2), ceil(grid_struct.my/2), i, 1, k );
      end
      end
      clear q_data;

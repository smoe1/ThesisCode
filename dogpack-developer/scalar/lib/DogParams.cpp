...
void DogParams::set_kmax()
{
    ...
    int kmax1[] = {1, 2,  3,  4,  5, 6};
    int kmax2[] = {1, 3,  6, 10, 15   };
    int kmax3[] = {1, 4, 10, 20, 35   };
    int kmax4[] = {1, 5, 15, 35, 70   };
    switch(ndims)
    {
        case 1:
        ...
        case 2:
            ...
            kmax = kmax2[ get_space_order()-1 ];
            break;

        case 3:
            kmax = kmax3[ get_space_order()-1 ];

        case 4:
            kmax = kmax4[ get_space_order()-1 ];
            break;
...
void DogParams::set_kmax_divfree()
{
    ...
    {
        case 1:
            kmax_divfree = -1; // get_space_order()?
            break;
        case 2:
...
void DogParams::checkParameters()
{
    ...
    switch(ndims)
    {
        ...
        case 4:
            ...
            break;
        default:
            derr << " ERROR: number of dimensions must be set to 1, 2 or 4.\n\n";
    }
}
...
{
    // check space_order
    if (get_space_order()<1 || get_space_order()>3 )
    {
        derr << "invalid spatial accuracy,"
            << " must have space_order = 1, or 2 " << endl;
    }  
...
void DogParams::set_outputdir (const char* arg)
{
    free(outputdir);
    outputdir = strdup(arg);
}

void DogParams::parse_arguments(int argc,char**argv)
{
    int show_usage=0;
